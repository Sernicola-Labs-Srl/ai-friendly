...
- Resa esplicitamente opt-in l'estrazione dei campi ACF, per evitare la pubblicazione involontaria di valori non visibili nel frontend.
- Rinominata la telemetria locale in registro attività, rimossi gli identificativi utente, documentata la privacy e aggiunta la pulizia completa alla disinstallazione.
...
## 1.7.0
- Estrazione contenuti più robusta per builder/ACF con selezione a punteggio tra candidati (`builder`, `rendered`, `the_content`, `fallback`) e debug esteso in `?debug=1`
...
- Output `.md` servito come markdown raw (rimozione `esc_html()` sull'intero payload) per evitare entità HTML indesiderate
...
- UI admin:
  - severità visiva per warning/error diagnostici (`Critico`, `Attenzione`, `Info`)
...
## 1.6.3
- Fix robustezza serving `.md` per CPT pubblici con resolver URL più affidabile
- Supporto archivio CPT in Markdown (es. `/podcast.md`) quando `has_archive` è attivo
